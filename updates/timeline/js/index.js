...
function loadVersions(edition, panelDOM) {
	...
	if (versions.length > 0) {
		...
		for (let i = 0; i <= yearAmount; i++) {
			...
			tlYearDOM.style.height = (multi * YEAR_PX * 2) + "px";
			rulerYearDOM.style.height = (multi * YEAR_PX * 2) + "px";
...
function makePreview(version) {
	...
	if (hasTitle || hasSubtitle || hasDesc) {
		...
		if (hasTitle && hasSubtitle) {
			textBoxDOM.appendChild(make("h2", "title subtitle", `${version.title} • ${version.subtitle}`));
		} else {
			if (hasTitle) {
...
function makeBox(version, edition, date) {
	...
	if (version.first) {
		...
		let text2 = make("h2", "", MONTH_NAMES[date.getMonth()] + " " + date.getFullYear());
		let box = make("div", "box center");
	...
	if (version.today) {
		...
		let text1 = make("h1", "", MONTH_NAMES[date.getMonth()] + " " + date.getFullYear());
		let text2 = make("h2", "", "Today");
		let box = make("div", "box center");
